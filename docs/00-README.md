...
- 真实信息辅助：Open-Meteo 天气前置到生成流程；可选高德 POI/同日相邻路线核验；特殊景观概率提示、景点高德入口、当前位置显示。
- 预订信息：票务/酒店截图 OCR → DeepSeek 结构化提取 → 可编辑 → 纳入排程；带价格时可自动记账。
- 账号与数据：Supabase 邮箱登录；手机号验证码/手机号密码前端流程已完成，短信服务启用后自动开放；行程收藏、消费账本、景点打卡、评分、推荐/踩坑点评、照片云存储。
...
- 模型返回已增加运行时行程结构校验；`pnpm test` 覆盖令牌、限流、请求参数、行程结构、天气、高德事实解析、票据脱敏和手机号规范化，8 项通过。
...
**当前缺口**：
- 手机验证码、手机密码注册/登录及找回密码界面已完成，但 Supabase Production 的 Phone Provider 尚未启用；需完成短信供应商、签名/模板审核和防刷配置后开放。
